# Staging: small instance classes, single-AZ, shorter retention. Mirrors prod
# topology (same modules) so staging is a faithful rehearsal environment —
# only sizing/HA toggles differ.
...
module "network" {
  ...
  azs                = var.azs
  single_nat_gateway = true # staging: one NAT gateway is enough, saves ~$32/mo
...
module "storage" {
  ...
  # Shorter noncurrent-version window than prod — staging churns test uploads.
  noncurrent_version_expiration_days = 30
  noncurrent_version_transition_days = 14
  tags                               = local.common_tags
}
...
module "email" {
  ...
  dmarc_policy        = "none" # staging: monitor only, never enforce
...
module "secrets" {
  ...
  recovery_window_days = 0 # staging: allow immediate secret deletion during teardown/recreate cycles
...
module "app" {
  ...
  # Staging: minimal footprint, small scaling ceilings.
  web_cpu       = 256
  web_memory    = 512
  api_cpu       = 256
  api_memory    = 512
  ...
  web_autoscaling_min    = 1
  web_autoscaling_max    = 2
  api_autoscaling_min    = 1
